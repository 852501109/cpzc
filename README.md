...
3. 将 src/vite-env.d.ts 添加到 tsconfig 中
...
通过`unplugin-vue-components`按需引入后，可以直接在.vue 文件模板中使用，并自动生成`components.d.ts`类型声明文件，js 中仍然需要手动引入组件
...
这里采用的是浏览器 API：`navigator.geolocation`
